                ...
                if  room.doors[d] is not None:
                    if d in [0, 2]:  # left or right
                        res_gates[gate_id] = {"direction": directions_str[d],
                                            ...
                                            "color":room.doors[d].color}
                    elif d in [1, 3]:  # up or down
                        res_gates[gate_id] = {"direction": directions_str[d],
...
def output_obs(room_array,output_file):
    with open(output_file, 'wb') as f:  
        #object,color,state [column,row,emb] 
        np.save(file=f,arr=room_array)
    #print(room_array)
...
def main():
    ...
    for i in range(500):
        ...
        for row in range(3):
            for col in range(3):
                room_array.append(obs[7*col:7*col+8,7*row:7*row+8])
        output_file='./obs_eval500'+'/obs{}.txt'.format(i)
        output_obs(room_array,output_file)
        env.render()
        pygame.image.save(env.window, "pic_test/obs{}.png".format(i))
        #print(obs["image"][2])
        #data=np.load(file=output_file)
