...
class ELU_A(nn.Module):
    def __init__(self):
        super(ELU_A, self).__init__()
        self.voc_embedding = nn.Embedding(Param.voc_size + 2, Param.voc_emb_size)
        #batch_first把batch放在第一维
        self.sent_encoder = nn.GRU(Param.voc_emb_size, 20, batch_first=True)
        self.history_encoder = nn.GRU(20, 20, batch_first=True)
    ...
    def _encode_message(self, message):
        ...
        hx = torch.reshape(hx, (msg_shape[0], msg_shape[1], 20))
    ...
    def forward(self, env_emb, message, choose_room_method="sample", obj_nums=None):
        ...
        if choose_room_method == "greedy":
            ...
            action_sampler = torch.distributions.Categorical(scores)
            action_idx = action_sampler.sample()
            # record actions
            action_prob.append(action_sampler.log_prob(action_idx))
        return action_idx, action_prob


    def backward(self, action_prob, reward):
        loss = -sum([action_prob[i] * reward[i] for i in range(len(action_prob))])
        loss.backward()

    def cal_loss(self, action_prob, reward):
        loss = -sum([action_prob[i] * reward[i] for i in range(len(action_prob))])
        return loss
...
class ELG_A(nn.Module):
    ...
    def forward(self, cur_room_emb, max_length, choose_token_method="sample"):
        ...
        #0表示在张量最外层加一个中括号变成第一维，也就是变成(1,batch_size,room_emb_size)(1,50,64)
...
class ELG_B(nn.Module):
    def __init__(self):
        super(ELG_B, self).__init__()
        #词汇表大小，每个词汇维度
        self.voc_embedding = nn.Embedding(Param.voc_size, Param.voc_emb_size)  # start token, and end token
        self.gru = nn.GRU(input_size=Param.voc_emb_size, hidden_size=Param.room_emb_size)
        #序贯模型
    ...
    def forward(self, cur_room_emb, max_length, choose_token_method="sample"):
        ...
        for i in range(2):
            ...
            spoken_token.append(token_idx)
            token_before = self.voc_embedding(token_idx).unsqueeze(0)
        spoken_token = torch.reshape(torch.cat(spoken_token, axis=0), (2, Param.batch_size))
        spoken_token = torch.transpose(spoken_token, 0, 1)
        assert spoken_token.shape[0] == Param.batch_size
        return spoken_token, spoken_token_prob
...
class ELU_B(nn.Module):
    ...
    def cal_loss(self, room_prob, reward):
        ...
        return loss
    '''
    def cal_loss(self, room_prob, tgt_rooms_arr):
            # room_prob: (batch_size, num_rooms)
            # labels: (batch_size,)
            
            # Apply log_softmax to the room probabilities
            log_probs = self.log_softmax(room_prob)
            
            # Calculate the cross-entropy loss
            loss = nn.functional.nll_loss(log_probs, tgt_rooms_arr)
...
class AgentA(nn.Module):
    def __init__(self, lang_understand=None, lang_generate=None):
        ...
        self.lang_understand = ELU_A() if lang_understand is None else lang_understand
...
class AgentB(nn.Module):
    def __init__(self, lang_understand=None, lang_generate=None, is_cal_all_route_init=False):
        ...
        self.lang_generate = ELG_B() if lang_generate is None else lang_generate
    ...
    def describe_action(self, ordered_gate_emb, max_length, choose_token_method="sample"):
        return self.lang_generate(ordered_gate_emb,  choose_token_method)

    def cal_guess_room_loss(self, room_prob, reward):
        return self.lang_understand.cal_loss(room_prob, reward)

    def cal_guess_action_loss(self, spoken_token_prob, reward):
        return self.lang_generate.cal_loss(spoken_token_prob, reward)
